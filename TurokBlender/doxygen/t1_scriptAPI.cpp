...
public:
    ...
    kFx @SpawnProjectile(const kStr&in fxPath, const kVec3&in pos, const kVec3&in targetLocation, const float maxAngle); ///< Returns the first Fx spawned. targetLocation.z gets added by GAME_SCALE * 3.0f
...
public:
    ...
    void SetWalkMaxSpeed(const float speed);           ///< default = 4.0f
    ...
    const float WalkMaxSpeed() const;           ///< default = 4.0f
...
public:
    ...
    float &MovingForce(); ///< initial velocity.Length() value. if bBullet is true then while updating movement OnTick, if the velocity.length() of the fx is below MovingForce() * 0.65f then removes the Fx.
...
public:
    ...
    void OverrideBloom(const int value); ///< 0=none, 1=force disable, 2=force enable
